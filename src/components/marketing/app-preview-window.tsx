...
type PreviewRow =
  | { card: Card; player: Player; label?: string; imageUrl?: never; imageAlt?: never; href?: string }
  | { label: string; imageUrl: string; imageAlt: string; href: string; card?: never; player?: never };
    ...
    <div className={styles.previewWindow}>
      ...
      <div className={styles.previewBody}>
        ...
        <div className={styles.previewMain}>
          ...
          <div className={styles.previewCards}>{featured.map((item) => {
            if (item.imageUrl) {
              return <Link href={item.href} key={item.label} className={styles.previewCard}>
                <div className={styles.featuredImage}><Image src={item.imageUrl} alt={item.imageAlt} fill sizes="(max-width: 620px) 30vw, 180px" /></div>
                <span>{item.label}</span>
              </Link>;
            }
            if (!item.card || !item.player) return null;
            const { card, player } = item;
            const label = player.displayNameZh;
            const href = `/cards/${card.id}`;
            return <Link href={href} key={`${label}-${card.id}`} className={styles.previewCard}>
